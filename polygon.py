...
class SoftPolygonFunction(Function):
    ...
    def forward(ctx, vertices, width, height, inv_smoothness=1.0, mode=MODE_MASK):
        ...
        rasterized = (
            torch.FloatTensor(ctx.batch_size, ctx.height, ctx.width)
            .fill_(0.0)
            .to(device=ctx.device)
        )

        contribution_map = (
            torch.IntTensor(ctx.batch_size, ctx.height, ctx.width)
            .fill_(0)
            .to(device=ctx.device)
        )
    ...
    def backward(ctx, grad_output):
        ...
        grad_vertices = (
            torch.FloatTensor(ctx.batch_size, ctx.number_vertices, 2)
            .fill_(0.0)
            .to(device=ctx.device)
        )
...
class SoftPolygon(nn.Module):
    ...
    def forward(
        ...
        return SoftPolygonFunction.apply(
            vertices, width, height, self.inv_smoothness, self.mode
        )
...
def pnp(vertices, width, height):
    ...
    for vn in range(polygon_dimension):
        ...
        has_condition = torch.logical_and(
            (from_y > yp) != (to_y > yp),
            xp < ((to_x - from_x) * (yp - from_y) / (to_y - from_y) + from_x),
        )
...
class SoftPolygonPyTorch(nn.Module):
    ...
    def forward(self, vertices, width, height, p, color=False):
        ...
        for from_index in range(polygon_dimension):
            ...
            square_segment_length = (
                x2_sub_x1 * x2_sub_x1 + y2_sub_y1 * y2_sub_y1 + 0.00001
            )
            ...
            dot = (
                (x_sub_x1 * x2_sub_x1) + (y_sub_y1 * y2_sub_y1)
            ) / square_segment_length
            ...
            segment_result[from_closest] = (
                x_sub_x1[from_closest] ** 2 + y_sub_y1[from_closest] ** 2
            )
            segment_result[to_closest] = (
                x_sub_x2[to_closest] ** 2 + y_sub_y2[to_closest] ** 2
            )
            segment_result[interior_closest] = (
                x_proj[interior_closest] ** 2 + y_proj[interior_closest] ** 2
            )
        ...
        F_theta = torch.gather(
            torch.stack(over_segments, dim=-1), dim=-1, index=F_arg.unsqueeze(-1)
        )[..., 0]
