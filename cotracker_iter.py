...
from cotracker import DAVIS_Video, CoEvalerIter, CotrackerIter, IterVideoLoss
import os
import logging
...
for e in range(epoch_num):
    ...
    for total_imgs, total_masks, total_points in train_video_loader:
        ...
        loss, iou = iter_video_loss(pred_points, target_masks.cuda())
        loss.backward()
        optimizer.step()
    ...
    logging.info(f"Epoch {e}, Loss: {loss_dict[e]:.4f}, Train IoU: {train_iou_dict[e]:.4f}")
    if e % eval_interval == 0 or e == epoch_num - 1:
        avg_iou = coevaler_iter.eval_all_video(model).item()
