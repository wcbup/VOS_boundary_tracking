...
# sample the second quarter of the training dataset

sample_save_dataset(
    video_dataset=train_rawset.data_set[
        len(train_rawset.data_set) // 4 : len(train_rawset.data_set) // 2
    ],
    start_idx=len(train_rawset.data_set) // 4,
    save_title="train",
    max_point_num=256,
    use_std_loss=True,
    fir_epoch_multi=5,
