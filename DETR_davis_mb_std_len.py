...
import logging
import json
from MyLoss import deviation_loss, get_edges, total_len_loss
from tenLoader import normalize

model_name = "DETR_davis_mb_std_len"
...
for interval in range(interval_steps):
    for e in range(interval_epochs):
        ...
        for (
            ...
            loss = 0.5 * dif_loss + 0.25 * std_loss + 0.25 * len_loss
            loss.backward()
            optimizer.step()
        ...
        logging.info(
            f"Epoch {epoch_index}, Total Loss: {mean_total_loss:.4f}, Dif Loss: {mean_dif_loss:.4f}, Std Loss: {mean_std_loss:.4f}, Len Loss: {mean_len_loss:.4f}",
        )
        epoch_index += 1
